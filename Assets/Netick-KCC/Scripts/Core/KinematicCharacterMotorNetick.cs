...
public class KinematicCharacterMotorNetick : MonoBehaviour
{
#pragma warning disable 0414
    ...
    [Header("Bug Fixes")]
    public bool FixInternalKccBugs = true;
...
#pragma warning restore 0414
    ...
    public int CharacterCollisionsOverlap(Vector3 position, Quaternion rotation, Collider[] overlappedColliders, float inflate = 0f, bool acceptOnlyStableGroundLayer = false)
    {
        ...
        int nbHits = 0;
        int nbUnfilteredHits = _PhysicsScene.OverlapCapsule(
                    bottom,
                    top,
    ...
    public int CharacterCollisionsSweep(Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, float inflate = 0f, bool acceptOnlyStableGroundLayer = false)
    {
        ...
        // Capsule cast
        int nbHits = 0;
        int nbUnfilteredHits = _PhysicsScene.CapsuleCast(
                bottom,
                top,
                Capsule.radius + inflate,
                direction,
    ...
    {
        closestHit = new RaycastHit();
        //Vector3 CapsuleTop = position + (rotation * _characterTransformToCapsuleTopHemi) - (direction * GroundProbingBackstepDistance);
        Vector3 CapsuleTop = position + (rotation * _characterTransformToCapsuleTopHemi) - (direction * GroundProbingBackstepDistance);
        //this bug prevented grounding from working, if there was an overhang that was the same collider as ground.
        //we fix this bug by starting the capsule cast (for ground sweeps) at the capsules CENTER instead of TOP
        if (FixInternalKccBugs)
            CapsuleTop = position + (rotation * _characterTransformToCapsuleCenter) - (direction * GroundProbingBackstepDistance);
        Vector3 CapsuleBottom = position + (rotation * _characterTransformToCapsuleBottomHemi) - (direction * GroundProbingBackstepDistance);
        //Debug.DrawLine(
        //    CapsuleBottom + Vector3.down * Capsule.radius,
        //    CapsuleTop + Vector3.up * Capsule.radius,
        //    Color.green, Time.fixedDeltaTime);
        //Debug.DrawLine(
        //    CapsuleBottom,
        //    CapsuleBottom + Vector3.down * (distance + GroundProbingBackstepDistance),
        ...
        for (int i = 0; i < nbUnfilteredHits; i++)
        {
            ...
            //Debug.DrawLine(hit.point, hit.point + hit.normal * 10, Color.blue, Time.fixedDeltaTime);
    ...
    public int CharacterCollisionsRaycast(Vector3 position, Vector3 direction, float distance, out RaycastHit closestHit, RaycastHit[] hits, bool acceptOnlyStableGroundLayer = false)
    {
        ...
        // Raycast
        int nbHits = 0;
        int nbUnfilteredHits = _PhysicsScene.Raycast(
            position,
            direction,
            hits,
            distance,
